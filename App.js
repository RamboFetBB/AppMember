...
export default function App() {
  var [abaInferior, setAbaInferior] = useState('gerador');
  var [subAba, setSubAba] = useState('fixos');
  var [eventos, setEventos] = useState(EVENTOS_FIXOS_INICIAIS);
  var [agora, setAgora] = useState(new Date());
  var [cooldowns, setCooldowns] = useState({});
  var [eventosCustom, setEventosCustom] = useState([]);
  var [novoNome, setNovoNome] = useState('');
  var [novoHorario, setNovoHorario] = useState('');
  ...
  async function alternarCooldown24h(slot) {
    ...
    if (estadoAtual && estadoAtual.ativo) {
      ...
      var notifId = await agendarNotificacao(
        ...
        'Ja passou 24h desde a ultima guilda',
        COOLDOWN_24H
      );
